...
print ('El tiempo de ejecucion sin gráficas  fue: ',tiempo_ejecucion1 ,"segundos")
########## Histogramas 
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D

#gráfica de  "x_test" vs "y_test" analisis manual
#para coorroborar que se está tomando menos datos que el test
plt.figure(figsize=(10,6))
plt. plot(x_test,y_test)
plt.xlabel("x test (Valor al aplicar TF-IDF)")
plt.ylabel("y test (Sentimiento)")
plt.grid() # para poner recuadros
plt.savefig('GR_test.jpg')

#gráfica de  "x_test vs "y_pred" 
#mostrar la gráfica  de las prediccones  con las x
plt.figure(figsize=(10,6))
plt. plot(x_test,y_pred)
plt.xlabel("x test (Valor al aplicar TF-IDF)")
plt.ylabel("y predecida (Sentimiento)")
plt.grid() # para poner recuadros
plt.savefig('GR_pred.jpg')

tiempo_final2 = time()
tiempo_ejecucion = tiempo_final2 - tiempo_inicial
print ('El tiempo de ejecucion con gráficas  fue: ',tiempo_ejecucion ,"segundos")#En segundos